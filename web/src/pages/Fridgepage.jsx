...
import { deleteFromFridge, getFridge } from "../api/foods";
import Toast from "../components/Toast";
import CategoryBadge from "../components/fridgepage/CategoryBadge";
...
export default function FridgePage() {
    ...
    const totalKcal = Math.round(
        foods.reduce((s, f) => s + (f.calories_per_100g ?? 0) * f.weight_g / 100, 0)
    );
    const avgW = foods.length
        ? Math.round(foods.reduce((s, f) => s + f.weight_g, 0) / foods.length)
        : 0;
    ...
    return (
        <div style={{ padding: "24px 28px", display: "flex", flexDirection: "column", gap: 20 }}>
            ...
            {/* Stats */}
            <StatsPanel count={foods.length} totalKcal={totalKcal} />
                ...
                {!loading && !error && (
                    <table style={{ width: "100%", borderCollapse: "collapse" }}>
                        ...
                        <tbody>
                            {filtered.length === 0 && (
                                <tr>
                                    <td colSpan={8} style={{ padding: 40, textAlign: "center", color: "#bbb", fontSize: 13 }}>
                            ...
                            {filtered.map((f) => (
                                ...
                                >
                                    ...
                                    <td style={tdStyle}>{f.weight_g}</td>
